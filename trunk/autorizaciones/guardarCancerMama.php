...
if($delcod != 0 and $delcod != NULL) {
	if(isset($_POST)) {
		...
		try {
			...
			$sqlCarga = "INSERT INTO cancermama (delcod,profesional,fechaatencion,nrcuil,nrafil,codpar,nombre,ddntelefono,nrotelefono,edad,antecedentes,personaantecedente,examenmamario,ultimoexamenmamario,mamografia,ultimamamografia,diagnostico,subdiagnostico,observaciones) VALUES (:delcod,:profesional,:fechaatencion,:nrcuil,:nrafil,:codpar,:nombre,:ddntelefono,:nrotelefono,:edad,:antecedentes,:personaantecedente,:examenmamario,:ultimoexamenmamario,:mamografia,:ultimamamografia,:diagnostico,:subdiagnostico,:observaciones)";
			$resCarga = $dbh->prepare($sqlCarga);
			if($resCarga->execute(array(':delcod' => $delcod, ':profesional' => strtoupper($_POST['profesional']), ':fechaatencion' => fechaParaGuardar($_POST['fechaatencion']), ':nrcuil' => $_POST['nrcuil'], ':nrafil' => $_POST['nrafil'], ':codpar' => $_POST['codpar'], ':nombre' => strtoupper($_POST['nombre']), ':ddntelefono' => $_POST['ddntelefono'], ':nrotelefono' => $_POST['nrotelefono'], ':edad' => $_POST['edad'], ':antecedentes' => $_POST['antecedentes'], ':personaantecedente' => $personaantecedente, ':examenmamario' => $_POST['examenmamario'], ':ultimoexamenmamario' => $ultimoexamenmamario, ':mamografia' => $_POST['mamografia'], ':ultimamamografia' => $ultimamamografia, ':diagnostico' => $_POST['diagnostico'], ':subdiagnostico' => $_POST['subdiagnostico'], ':observaciones' => $_POST['observaciones'])))
			$dbh->commit();
