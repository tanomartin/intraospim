...
if($delcod != 0 and $delcod != NULL) {
	if(isset($_POST)) {
		...
		try {
			...
			$sqlCarga = "INSERT INTO hipertension (delcod,profesional,fechaatencion,nrcuil,nrafil,codpar,nombre,ddntelefono,nrotelefono,edad,talla,peso,presion,antecedenteshipertension,personaantecedentehipertension,antecedentescardiacos,personaantecedentecardiaco,emitediagnostico,diagnostico,subdiagnostico,observaciones) VALUES (:delcod,:profesional,:fechaatencion,:nrcuil,:nrafil,:codpar,:nombre,:ddntelefono,:nrotelefono,:edad,:talla,:peso,:presion,:antecedenteshipertension,:personaantecedentehipertension,:antecedentescardiacos,:personaantecedentecardiaco,:emitediagnostico,:diagnostico,:subdiagnostico,:observaciones)";
			$resCarga = $dbh->prepare($sqlCarga);
			if($resCarga->execute(array(':delcod' => $delcod, ':profesional' => strtoupper($_POST['profesional']), ':fechaatencion' => fechaParaGuardar($_POST['fechaatencion']), ':nrcuil' => $_POST['nrcuil'], ':nrafil' => $_POST['nrafil'], ':codpar' => $_POST['codpar'], ':nombre' => strtoupper($_POST['nombre']), ':ddntelefono' => $_POST['ddntelefono'], ':nrotelefono' => $_POST['nrotelefono'], ':edad' => $_POST['edad'], ':talla' => $_POST['talla'], ':peso' => $_POST['peso'], ':presion' => $_POST['presion'],':antecedenteshipertension' => $_POST['antecedenteshipertension'], ':personaantecedentehipertension' => $personaantecedentehipertension, ':antecedentescardiacos' => $_POST['antecedentescardiacos'], ':personaantecedentecardiaco' => $personaantecedentecardiaco, ':emitediagnostico' => $_POST['emitediagnostico'], ':diagnostico' => $_POST['diagnostico'], ':subdiagnostico' => $_POST['subdiagnostico'], ':observaciones' => $_POST['observaciones'])))
			$dbh->commit();
